...
UTILS_SRCS = $(wildcard $(SRCSDIR)utils/*.c)
UTILS_OBJS = $(UTILS_SRCS:$(SRCSDIR)%.c=$(SRCSDIR)%.o)

MAIN_SRC = $(SRCSDIR)main.c
MAIN_OBJ = $(MAIN_SRC:$(SRCSDIR)%.c=$(SRCSDIR)%.o)

SRCS = $(INIT_SRCS) $(PLAYER_SRCS) $(MAIN_SRC) $(ERROR_SRCS) $(UTILS_SRCS)
OBJS = $(INIT_OBJS) $(PLAYER_OBJS) $(MAIN_OBJ) $(ERROR_OBJS) $(UTILS_OBJS)
